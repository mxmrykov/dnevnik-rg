// @ts-ignore
import {MicroClasses, MicroClassesMap, ShortClassInfo} from "../../constants/class.ts";
// @ts-ignore
import * as constants from "../../constants/api.ts";
// @ts-ignore
import {GetMonthClass} from "../../constants/response.ts";
import axios from "axios";

export default async function GetClassesMonth(route: string):
    Promise<{ error: boolean, message: string, classes: MicroClassesMap[] }> {
    return await constants.instance.get<GetMonthClass>(
        route,
        {
            headers: {
